...
- 在任意网页（包括 YouTube）或字幕中选中文字，即可自动翻译为简体中文。
...
5. 选中字幕，获取 AI 内容讲解。
6. 在任意网页用鼠标拖选文字，选区旁边会出现中文翻译卡片。也可以右键选中文字，再点菜单里的 **翻译成中文**。
7. 从播放器或重点引用中保存笔记，之后可以在 **Notes** 中查看。

## 当前支持范围

- Chrome 116 或更高版本。
- 标准的 `youtube.com/watch` 视频页面，用于字幕、概览和笔记。
- 在普通 http(s) 网页（包括 YouTube）上选中文字，即可按需翻译为简体中文。
- Supadata 能够返回的原生字幕。YouTube Digest 会优先请求英文字幕，也可能显示其他可用的原生语言。
- 原文、简体中文和双语对照字幕。
- AI 概览、选中文本讲解、选中文字即时中文翻译和自动润色笔记。
